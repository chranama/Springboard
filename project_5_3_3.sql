...
SELECT name AS facility_name FROM `Facilities` WHERE membercost = 0
...
SELECT CONCAT(f.name, ' ', m.firstname, ' ', m.surname)
    AS tennis_courts_bookings
FROM ((`Bookings` b
    JOIN `Facilities` f
        ON b.facid = f.facid)
    JOIN `Members` m
